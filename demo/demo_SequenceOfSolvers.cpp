...
int main()
{
	...
	using SolverSequence = LNOT::SequenceOfSolvers<LNOT::SR1TrustRegionSolver<LanczosTRS>, LNOT::NewtonTrustRegionSolver<LanczosTRS>, LNOT::NewtonSolver<CG, BisectLS> >;
	
	static_assert(SolverSequence::order == 2);
	
	SolverSequence solverSeq(100); 
	
	solverSeq.setOutput(stdout);
	solverSeq.solve(func, grad, hessOp, precOp, N, x);
	
	fmt::print("Newton with Bisection LineSrearch found : {:.2f} in {} iterations with a final error of {} and f(x) = {}\n", fmt::join(x_view, " "), solverSeq.getIterations(), solverSeq.getError(), solverSeq.getValue());
